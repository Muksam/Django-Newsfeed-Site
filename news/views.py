...
def index(request):
    content = {
        ...
        'newsDetailsData': News.objects.all(),
...
def slider_details(request, slug):
    content = {
        'sliderData': Slider.objects.filter(slug=slug)
    }

    return render(request, 'pages/slider-details.html', content)

def latestnews_details(request, slug):
    content = {
        'latestnewsData': LNews.objects.filter(slug=slug)
    }

    return render(request, 'pages/latestnews-details.html', content)
...
def contact (request):
    if request.method == 'POST':
       ...
       if form.is_valid():
           form.save()
           messages.success(request,'you have sucessfully contacted')
